...
class Gek_Logger
{
    ...
    {
        //获取配置
        $loggerConfig = Yaf_Registry::get("config")->get("logger");
        if (!isset($loggerConfig)) {
            throw new Exception('Sorry, logger server is busy now, please try again later (001)');
        }
        self::$_CONFIG = $loggerConfig;
        $config = self::$_CONFIG->toArray();
        //日志文件
        $config['logfile'] = $tag.'.log';
        // 设置标签
        $config['tag'] = $tag!='' && $tag!='default'? $tag : '-';
        // 返回日志类对象
        return new Gek_Logger($config);
    ...
    public function __construct($config = array())
    {
        ...
        if (isset($config['logpath'])) {
            $this->_log_path = $config['logpath'];
        }
        // 日志文件
        if (isset($config['logfile'])) {
            $this->_log_file = $config['logfile'];
        }
        // 日志自定义目录
